    ...
    <div className="min-h-screen bg-[#0a0a0a]">
        ...
        <div className="container mx-auto px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="grid md:grid-cols-2 gap-8">
            {teamMembers.map((member, index) => (
              <motion.div key={index} variants={fadeInUp} className="group bg-[#141414] rounded-lg border border-[#2a2a2a] overflow-hidden hover:border-[#C9A962]/30 transition-all duration-300">
                <div className="grid sm:grid-cols-[200px_1fr] gap-0">
                  <div className="relative h-64 sm:h-full overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-[#141414]/50 to-transparent sm:bg-gradient-to-r" />
                  </div>
                  <div className="p-6">
                    <h3 className="font-display text-xl font-semibold text-[#FAFAF5] mb-1">{member.name}</h3>
                    <p className="font-accent text-sm text-[#C9A962] mb-4">{member.title}</p>
                    <p className="font-body text-sm text-[#FAFAF5]/60 mb-4 leading-relaxed">{member.bio}</p>
                    <div className="space-y-3">
                      <div>
                        <span className="font-accent text-xs text-[#FAFAF5]/40 uppercase tracking-wider">Expertise</span>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {member.expertise.split(", ").map((skill, i) => (
                            <span key={i} className="text-xs font-body px-2 py-0.5 bg-[#C9A962]/10 text-[#C9A962] rounded">{skill}</span>
                          ))}
                        </div>
                      </div>
                      <div>
                        <span className="font-accent text-xs text-[#FAFAF5]/40 uppercase tracking-wider">Regions</span>
                        <p className="font-body text-sm text-[#FAFAF5]/60 mt-1">{member.regions.split(", ").join(" · ")}</p>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <section className="py-24 bg-[#0d0d0d]">
        <div className="container mx-auto px-6 text-center">
          <h2 className="font-display text-3xl md:text-4xl font-semibold text-[#FAFAF5] mb-6">Work With Our Team</h2>
