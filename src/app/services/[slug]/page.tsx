...
import { getServiceBySlug, getRelatedServices, ICON_MAP } from "@/data/services";
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-display text-4xl text-[#FAFAF5] mb-4">Service Not Found</h1>
          <p className="font-body text-[#FAFAF5]/60 mb-8">The service you&apos;re looking for doesn&apos;t exist.</p>
          <Link href="/#services" className="font-body text-[#C9A962] hover:underline">View All Services</Link>
        </div>
      </div>
    );
  }
        ...
        <div className="relative z-10 container mx-auto px-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="flex items-center gap-4 mb-6">
              {IconComponent && (
                <div className="inline-flex items-center justify-center w-14 h-14 rounded-lg bg-[#C9A962]/20">
              ...
              <div className="space-y-4">
                {service.benefits.map((benefit, index) => (
                  <motion.div key={index} variants={fadeInUp} className="flex items-start gap-3">
                    <CheckCircle className="h-5 w-5 text-[#C9A962] mt-0.5 flex-shrink-0" />
              ...
              <div className="space-y-6">
                {service.approach.map((step, index) => (
                  <motion.div key={index} variants={fadeInUp} className="flex gap-4">
                    <span className="font-accent text-2xl font-semibold text-[#C9A962]/30">{String(index + 1).padStart(2, "0")}</span>
                    <div>
                ...
                return (
                  <Link key={index} href={`/services/${related.slug}`} className="group p-6 bg-[#141414] rounded-lg border border-[#2a2a2a] hover:border-[#C9A962]/30 transition-all duration-300">
                    <div className="flex items-start gap-4">
                      {RelatedIcon && (
                        <div className="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-[#C9A962]/10 group-hover:bg-[#C9A962]/20 transition-colors flex-shrink-0">
